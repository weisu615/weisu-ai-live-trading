...
## Binance futures market-data note

- This app must use Binance USD-M perpetual futures K-lines from `/fapi/v1/klines`.
- Do not fall back to Binance spot data and do not generate synthetic candles.
- If Render shows HTTP 451 from Binance, the current deployment region/IP is blocked for futures market-data access.
- Fix HTTP 451 by redeploying the service in a non-restricted region, or by setting `BINANCE_FAPI_BASES` to trusted proxy origins that serve the same USD-M futures `/fapi/v1/klines` path.
